...
2. **월~금(mon-fri) 5일은 반드시 포함**하고, 토/일은 선택적으로 추가하세요.
3. 각 요일에 **1-3개 노드를 배정**하되, 각 노드는 위 "학습 가능한 노드" 목록에서 **ID를 정확히 복사**하세요.
   목록에 없는 노드 ID를 만들지 마세요.
4. 같은 노드를 여러 요일에 중복 배정해도 됩니다 (복습 개념).
5. 약점 영역의 개념을 보완할 수 있도록 우선순위를 조정하세요.
6. 각 요일의 학습 이유를 2-3문장으로 명확히 설명하세요.
7. 마지막에 동기부여 메시지를 한 문단 작성하세요.
8. 모든 텍스트는 한국어로 작성하세요.
`
